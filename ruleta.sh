...
martingala() {
  ...
  echo -ne "${yellowColour}[+] ${endColour}${grayColour}¿Cuál es la cantidad a apostar? -> ${endColour}" && read -r initial_bet
  echo -ne "${yellowColour}[+] ${endColour}${grayColour}¿A qué deseas apostar continuamente (par/impar)? -> ${endColour}" && read -r par_impar
  # Ocultar el cursor
  tput civis
    ...
    if [ ! "$money" -lt 0 ]; then
      ...
      par)
        if [ "$(($random_number % 2))" -eq 0 ] && [ "$random_number" -ne 0 ]; then
          ...
          if [ $money -gt $limit_to_renew ]; then
            ...
            echo -e "${yellowColour}[+]${endColour} El tope se ha establecido en: ${yellowColour}$limit_to_renew${endColour}\$"
            my_sequence=(1 2 3 4)
      ...
      impar)
        if [ "$(($random_number % 2))" -ne 0 ] && [ "$random_number" -ne 0 ]; then
          echo -e "${yellowColour}[+]${endColour}${greenColour} El numero es impar${endColour}"
          reward=$(($bet * 2))
          let money+=$reward
          echo -e "${yellowColour}[+]${endColour} Tienes ${yellowColour}$money\$${endColour}"
          ...
          if [ "$(($random_number % 2))" -eq 0 ]; then
            echo -e "${yellowColour}[!]${endColour}${redColour} El número es par${endColour}"
          else
            echo -e "${yellowColour}[!]${endColour}${redColour} El número es cero${endColour}"
          fi
